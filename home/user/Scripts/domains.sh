	...
	$opt1 ) #Amass
		...
		if [ ! -z "$domain" ]; then
			...
			if [[ $domain =~ $fqdnregex ]]; then
				#Run Tool
				mkdir /root/Cases/Amass/
				amass intel -whois -ip -src -d $domain  -o /root/Cases/Amass/$timestamp-$domain.1.txt 
				amass enum -src -brute -d $domain -o /root/Cases/Amass/$timestamp-$domain.2.txt -d $domain
				sleep 3
				thunar "/root/Cases/Amass/" >/dev/null 2>&1
				exit
			else
		...
		if [ ! -z "$domain" ]; then
			...
			if [[ $domain =~ $fqdnregex ]]; then
				#Run Tool
				mkdir /root/Cases/Sublist3r/
				sublist3r -d $domain -o /root/Cases/Sublist3r/sublist3r_$domain.txt
			else
				zenity --error --text "Doesn't appear to be a legitimate domain, exiting!" 2> >(grep -v 'GtkDialog' >&2)
				exit
 			fi
		...
		if [ ! -z "$domain" ]; then
			...
			if [[ $domain =~ $fqdnregex ]]; then
				#Run Tool
				mkdir /root/Cases/Photon/
				photon -u $domain -l 3 -t 100 -o /root/Cases/Photon/$timestamp-$domain
				sleep 3
				thunar "/root/Cases/Photon/$timestamp-$domain" >/dev/null 2>&1
				exit
			else
				zenity --error --text "Doesn't appear to be a legitimate domain, exiting!" 2> >(grep -v 'GtkDialog' >&2)
				exit
		...
		if [ ! -z "$domain" ]; then
			...
			if [[ $domain =~ $fqdnregex ]]; then
				#Run Tool
				mkdir /root/Cases/theHarvester/
				theHarvester -d $domain -b all -f /root/Cases/theHarvester/$timestamp-$domain.html
				if [ -f /root/Cases/theHarvester/$timestamp-$domain.html  ]; then
					firefox /root/Cases/theHarvester/$timestamp-$domain.html	
				fi	
			else
				zenity --error --text "Doesn't appear to be a legitimate domain, exiting!" 2> >(grep -v 'GtkDialog' >&2)
				exit
		...
		$opt5 ) #De-Cloud
				...
				if [ ! -z "$domain" ]; then
					mkdir /root/Cases/De-Cloud
					#Run Tool
					cd /opt/csitools
					./decloud $domain | tee /root/Cases/De-Cloud/decloud-$domain.txt
					filename=/root/Cases/De-Cloud/decloud-$domain.txt
			
					FILE=/root/Cases/De-Cloud/tmp-$domain.txt
					if test -f "$FILE"; then
						cat /root/Cases/De-Cloud/decloud-$domain.txt | grep "Checking" | cut -d" " -f4 | sed 's/<BR>/\n/' | sort -u | tee -a /root/Cases/De-Cloud/tmp-$domain.txt
						cat /root/Cases/De-Cloud/tmp-$domain.txt | sed 's/<BR>/\n/' | sort -u > /tmp/tmp.txt
						cat /tmp/tmp.txt > /root/Cases/De-Cloud/tmp-$domain.txt 
						rm /tmp/tmp.txt	
					else
						cat /root/Cases/De-Cloud/decloud-$domain.txt | grep "Checking" | cut -d" " -f4 | sed 's/<BR>/\n/' | sort -u | tee /root/Cases/De-Cloud/tmp-$domain.txt
					fi
					gedit /root/Cases/De-Cloud/tmp-$domain.txt &		
